...
export class UserUseCase implements IuserUseCase{
  ...
  async googleLogin(email: string, name: string, password: string, next: NextFunction): Promise<{ user: Iuser; token: IToken; } | void> {
    ...
    if(!checkUser){
      password = await this.encrypt.createHash(password);
         ...
         if(user){
           if(user.isBlock){
             ...
             const token:any = await this.jwtToken.createAccessAndRefreashToken(user._id as string)
              if(token){
                return {user,token}
              }
           }
      ...
      if(checkUser.isBlock){
       ...
       const token:any = await this.jwtToken.createAccessAndRefreashToken(checkUser._id as string)
        if(token){
 ...
 async editUser(userId: string,name:string,email:string, next: NextFunction): Promise<Iuser | void> {
   ...
   const currentUser = await this.userRepository.findById(userId)
        ...
        const checkUser = await this.userRepository.findByEmail(email);
         if(checkUser){
          ...
          if(checkUser.email === currentUser.email){
            ...
            const updatedUser = await this.userRepository.update(userId,name,email)
            if(updatedUser){
              return updatedUser;
            }
          }else{
    ...
    async userSignUp(user:Iuser,next:NextFunction): Promise<string | void> {
        try {
            ...
            const token = await this.jwtToken.createVerificationJwt({name:user.name,email:user.email,password:user.password})
            return token;
    ...
    async insertUser(token:string,otp:string,next:NextFunction): Promise<Iuser | void> {
       try {
         const decoded = await this.jwtToken.verifyJwt(token) as Iuser
           ...
           const addedUser = await this.userRepository.create(decoded)
    ...
    async login(email:string,password:string,next:NextFunction): Promise<{user:Iuser,token:IToken} | void> {
      try {
         ...
         const checkPassword = await this.encrypt.comparePassword(password,user.password) 
    ...
    async verifyEmail(email:string,next:NextFunction): Promise<any | void> {
       try {
        ...
        if(checkuser){
          ...
          await this.sentEmail.sentEmailVerification(checkuser.name,checkuser.email,otp);
          return checkuser;
        }else{
          return next(new ErrorHandler(400,"Email Not Registerd"))
        }
    ...
    async verifyOtp(email:string,otp:string,next:NextFunction): Promise<any | void> {
       try {
    ...
    async changePassword(email:string,password:string,next:NextFunction): Promise<any | void> {
       try {
           const user = await this.userRepository.findByEmail(email)
           if(user){
